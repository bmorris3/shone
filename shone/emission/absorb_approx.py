...
def gauss_quad_mu(degree=2):
    ...
    Examples
    --------
    >>> weights, mu = gauss_quad_mu(degree=2)
    """
    if degree == 1:
...
@partial(jit, static_argnames=('T_int', 'atmos', 'weights', 'mu', 'flux_top_of_atmos'))
def compute_olr(T_int=temperatures.max(), atmos=atmos, mu=mu, weights=weights, flux_top_of_atmos=flux_top_of_atmos):
    # compute incoming fluxes in downward pass:

    @partial(jit, static_argnames=('atmos', 'mu'))
    def propagate_flux_down(carry, i, atmos=atmos, mu=mu):
        flux_down = atmos.I_down_2_carry(i, mu, carry)
        return flux_down, flux_down

    @partial(jit, static_argnames=('atmos', 'mu'))
    def propagate_flux_up(carry, i, atmos=atmos, mu=mu):
        (flux_up, dtau) = atmos.I_up_1_carry(i, mu, carry)
        return flux_up, dtau

    @partial(jit, static_argnames=('flux_top_of_atmos',))
    def body_fun(carry, mu, flux_top_of_atmos=flux_top_of_atmos):
        indices = jnp.arange(1, len(pressures))

        fluxes_boa, fluxes_down = lax.scan(
            lambda carry, i: propagate_flux_down(carry, i, mu=mu),
            flux_top_of_atmos[None, :],
            indices
        )

        flux_bottom_of_atmos = fluxes_boa + atmos.blackbody(T_int)

        # compute outgoing fluxes in upward pass:
        fluxes_toa, dtau = lax.scan(
            lambda carry, i: propagate_flux_up(carry, i, mu=mu),
            flux_bottom_of_atmos[None, :],
            indices[::-1]
        )
        return dtau, fluxes_toa.squeeze()

    init = jnp.empty((atmos.pressure.size - 1, atmos.wavelength.size))
    dtau, fluxes_TOA_mu = lax.scan(
        body_fun, init, mu.squeeze()
    )

    return jnp.sum(weights * fluxes_TOA_mu, axis=0), dtau, fluxes_TOA_mu